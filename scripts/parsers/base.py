...
class Message:
    ...
    # Пояснение инструмента к сообщению — почему вложения нет, что потерялось.
    # Не текст участника: экранированию как чужой текст не подлежит.
    note: str | None = None
    # Снимок того, на что дан ответ. Bot API не отдаёт историю группы, поэтому
    # вложенный родитель — единственный способ сохранить сообщение, пришедшее
    # до появления бота. Три вида: `message` (полный снимок), `external`
    # (текста нет, есть происхождение) и `quote` (только видимая цитата).
    ...
    def content_key(self) -> str | None:
        ...
        """
        # Сообщение с вложением опознаётся только основным ключом: разные люди
        # присылают разные файлы под одинаковым текстом («вот каталог»), и
        # схлопывать их значило бы потерять вложение.
        if self.media:
            return None
